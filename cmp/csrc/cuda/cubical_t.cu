...
__global__ void
enum_edges_t_2d_kernel(const PackedAccessorRestrict<scalar_t, 3> image,
  ...
  for (index_t m = 0; m < M; m++) {
    float birth = get_birth_t_2d(batch, y, x, 1, m, threshold);
    birth_b[m][y][x] = birth;
  }
}
...
template <typename scalar_t>
__global__ void
union_find_t_2d_d0_init_kernel(const PackedAccessorRestrict<scalar_t, 3> image,
  ...
  index_t idx = y * W + x;
  float birth = get_birth_t_2d(image[b], y, x);
...
template <typename scalar_t>
__global__ void
joint_pairs_t_2d_d0_kernel(const PackedAccessorRestrict<scalar_t, 2> values,
...
template <typename scalar_t>
__global__ void joint_pairs_t_2d_d0_out_parents_kernel(
  ...
  get_parent_t_2d(image_b, cy, cx, birth_val);
  cofaces_bs[0] = cy;
  cofaces_bs[1] = cx;

  if (death_idx != -1) {
    width_divmod(cy, cx, death_idx);
    get_parent_t_2d(image_b, cy, cx, death_val);
    cofaces_bs[2] = cy;
    cofaces_bs[3] = cx;
  } else {
...
torch::Tensor enum_edges_t_2d(const torch::Tensor &image,
                                           double threshold) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      image.scalar_type(), "enum_edges_t_2d", ([&] {
        cuda::t_cons::enum_edges_t_2d_kernel<scalar_t><<<blocks, threads>>>(
            image_contig
...
joint_pairs_t_2d_d0(const torch::Tensor &image, const torch::Tensor &values,
                    torch::Tensor &indices, const torch::Tensor &argmax_idx,
                    double threshold, int64_t block_size = 512) {
  constexpr index_t M = 2;
  ...
  at::Tensor lengths;
  {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        image.scalar_type(), "union_find_t_2d_d0_init", ([&] {
          cuda::t_cons::union_find_t_2d_d0_init_kernel<
...

    {
      ...
      do {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            image.scalar_type(), "joint_pairs_t_2d_d0", ([&] {
              cuda::t_cons::joint_pairs_t_2d_d0_kernel<scalar_t><<<blocks, threads>>>(
                  ...
                  aux_idx.packed_accessor32<index_t, 2,
                                            torch::RestrictPtrTraits>(),
                  max_end_idx_t.data_ptr<index_t>(), threshold, width_divmod,
                  m_divmod, first_iter);
            }));
  ...
  AT_DISPATCH_FLOATING_TYPES(
      image.scalar_type(), "joint_pairs_t_2d_d0_out_parents", ([&] {
        cuda::t_cons::joint_pairs_t_2d_d0_out_parents_kernel<
...
TORCH_LIBRARY_IMPL(cubical_ph, CUDA, m) {
  m.impl("enum_edges_t_2d", &enum_edges_t_2d);
  m.impl("joint_pairs_t_2d_d0", &joint_pairs_t_2d_d0);
}
